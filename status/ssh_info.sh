show_ssh_info() {
  ...
  local dir_icon=$(get_tmux_option "@kanagawa_directory_icon" "")

  local script_path="$CURRENT_DIR/utils/get_ssh_status.sh"

  # 2. Pass everything to the script
  # Order: PID, ABBR, BAR_BG, L_SEP, R_SEP, ICON, DIR, SHOW_DIR, DIR_ICON
  echo "#($script_path #{pane_pid} '$abbr' '$bar_bg' '$l_sep' '$r_sep' '$icon' '#{?pane_path,#{pane_path},#{pane_current_path}}' '$show_dir' '$dir_icon')"
}
